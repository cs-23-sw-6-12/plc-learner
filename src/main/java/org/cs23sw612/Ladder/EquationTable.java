...
/**
 * A table over the "equations" from a given machine. Generally, it represents a collection of all
 * the transitions (input, state, output, next state).
 *
 * @param <S> States
 * @param <I> Input
 * @param <T> Transitions. Can only be of input/output types, which (currently) is only {@link
...
class EquationTable<
    S,
    I,
    T extends CompactMealyTransition<O>,
    O,
    M extends MealyMachine<S, I, T, O>,
    A extends Alphabet<I>> {
  // TODO: Remove names?
  ...
  private String asString(String sep, String catSep, String lineSep, String headerSep) {
    ...
    String body =
        String.join(
            lineSep, equations.stream().map(row -> row.asString(sep, catSep, boolToInt)).toList());
    return header + lineSep + headerSep + body;
  }

  private String latexTableHeader() {
    return "|"
        + String.join("|", inputs.stream().map((i) -> "X").toList())
        + "||"
   ...
   * @param <O> Output
   */
  record EquationRow<S, I, O>(
      @NonNull I ins, @NonNull S states, @NonNull S nextStates, @NonNull O out) {
    @Override
    public String toString() {
