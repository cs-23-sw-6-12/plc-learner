...
/**
 * Finds transitions with uncertain clock guards and "trims" them to smallest
 * equivalent.
 *
 * <p>
 * Performs a complete exploration checking for outputs tagged with [?] clock
 * guards. The step clock limit is trimmed and output compared to see if the
 * trim still results in equivalence.
 *
 * <p>
 * Clock discovery walk: 1) For each outgoing transition check the guard on the
 * output and see if we can do it faster 2) The output must match the hypothesis
 * output 3) The successor state signature must match the hypothesis successor
 * state signature 4) If output and successor match then "trim" the clock guard
 *
 * <p>
...
public class ClockExplorationEQOracle<I, O> implements MealyEquivalenceOracle<I, O> {
	...
	static boolean outputsAreEquivalent(String observed, String expected) {
		...
		/*
		 * If the expected guard is null and the observed is > 0 this is an unobservable
		 * transition and the observed guard has just timed to the limit
		 */
	...
	HashMap<List<I>, Long> findUncertainPrefixes(CompactMealy<I, O> hypothesis, Collection<? extends I> inputs) {
		...
		// Store access prefixes resulting in uncertain clock guards in a hash map (no
		// duplicates?)
		...
		for (Integer state : hypothesis.getStates()) {
			...
			for (I sym : inputs) {
				...
				// if the last transition contains an uncertain clock guard then add it to the
				// list to trim
	...
	private <S, T> void trimClockGuard(MealyMachine<S, I, T, O> hypothesis, Collection<? extends I> inputs,
			Map.Entry<List<I>, Long> uncertainPrefix) {
		// The uncertain prefix comes as a hashmap pair with key: list of inputs <I> as
		// a prefix; value:
		...
		for (I sym : inputs) {
			try {
				...
				// check that the next symbol (after the prefix) has the same output as the
				// hypothesis
				String expectedOutput = hypothesis.getOutput(cur, sym).toString();
				// use the clock guard from hypothesis for SUL clock limit so we get identical
				// outputs
