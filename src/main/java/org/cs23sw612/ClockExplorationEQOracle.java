 ...
 *
 * <p>Performs a complete exploration checking for outputs tagged with [?] clock guards. The step
 * clock limit is trimmed and output compared to see if the trim still results in equivalence.
 *
 * <p>Clock discovery walk: 1) For each outgoing transition check the guard on the output and see if
 * we can do it faster 2) The output must match the hypothesis output 3) The successor state
 * signature must match the hypothesis successor state signature 4) If output and successor match
 * then "trim" the clock guard
 *
 * <p>Based on CompleteExplorationEQOracle by Malte Isberner.
 *
 * @param <I> input symbol class
 * @param <O> output class
 * @author Ben Caldwell <benny.caldwell@gmail.com>
 */
public class ClockExplorationEQOracle<I, O> implements MealyEquivalenceOracle<I, O> {
  ...
  private <S, T> DefaultQuery<I, Word<O>> doFindCounterExample(
    ...
    HashMap<List<I>, Long> uncertainPrefixes =
        findUncertainPrefixes((CompactMealy<I, O>) hypothesis, inputs);
  ...
  HashMap<List<I>, Long> findUncertainPrefixes(
      CompactMealy<I, O> hypothesis, Collection<? extends I> inputs) {
  ...
  List<? extends I> findAccessPrefix(
      Integer state, CompactMealy<I, O> hypothesis, Collection<? extends I> inputs) {
  ...
  private <S, T> void trimClockGuard(
      MealyMachine<S, I, T, O> hypothesis,
      Collection<? extends I> inputs,
      Map.Entry<List<I>, Long> uncertainPrefix) {
    // The uncertain prefix comes as a hashmap pair with key: list of inputs <I> as a prefix; value:
    // output <O> from the prefix
    ...
    for (I sym : inputs) {
      try {
        ...
        ListIterator<I> iterator =
            uncertainPrefix.getKey().listIterator(); // for each <I> in the prefix
        ...
        if (outputsAreEquivalent(observedOutput, expectedOutput)) {
          if (clockGuard <= 0L) {
            ...
            LOGGER.fine(
                "Trimmed clock guard to zero and removed: " + uncertainPrefix + "/" + newOutput);
            ((MutableTransitionOutput) hypothesis)
                .setTransitionOutput(uncertainTransition, newOutput);
          } else {
            // the trimmed guard did not affect the result so keep it trimmed and uncertain
            String newOutput =
                symbolFromOutput(uncertainOutput.toString())
                    + "[?"
                    + Math.floor(clockGuard / 1000.0f)
                    + "]";
            LOGGER.fine(
                "Trimmed clock guard is still uncertain: " + uncertainPrefix + "/" + newOutput);
            ((MutableTransitionOutput) hypothesis)
                .setTransitionOutput(uncertainTransition, newOutput);
          }
        } else {
          ...
          String newOutput =
              symbolFromOutput(uncertainOutput.toString())
                  + "["
                  + Math.floor(uncertainPrefix.getValue() / 1000.0f)
                  + "]";
          LOGGER.fine(
              "Trimmed clock guard uncertainty removed: " + uncertainPrefix + "/" + newOutput);
          ((MutableTransitionOutput) hypothesis)
              .setTransitionOutput(uncertainTransition, newOutput);
