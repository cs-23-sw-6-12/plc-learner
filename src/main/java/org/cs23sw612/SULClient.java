...
public class SULClient<I, IA extends InputAdapter<I>, O, OA extends OutputAdapter<O>> implements SULTimed<I, O> {
	...
	public static <I, IA extends InputAdapter<I>, O, OA extends OutputAdapter<O>> SULClient<I, IA, O, OA> createClient(
			Socket socket, IA inputAdapter, OA outputAdapter) throws IOException {
	...
	/// Creates a SULClient without a socket, that will just write to any
	/// PrintWriter and read from
	// any BufferedReader.
	public static <I, IA extends InputAdapter<I>, O, OA extends OutputAdapter<O>> SULClient<I, IA, O, OA> createClient(
			PrintWriter writer, BufferedReader reader, IA inputAdapter, OA outputAdapter) throws IOException {
		SULClient<I, IA, O, OA> client = new SULClient<I, IA, O, OA>();
		client.out = writer;
		client.in = reader;
		client.inputAdapter = inputAdapter;
	...
	public static <I, IA extends InputAdapter<I>, O, OA extends OutputAdapter<O>> SULClient<I, IA, O, OA> createClient(
			SULClientConfiguration configuration, IA inputAdapter, OA outputAdapter) throws IOException {
		// Configure socket.
		Socket socket = new Socket(configuration.address, configuration.port);
		return createClient(socket, inputAdapter, outputAdapter);
	}

	public static SULClient<String, StringInputAdapter, String, StringOutputAdapter> createStringClient(
			SULClientConfiguration configuration) throws IOException {
	...
	public static SULClient<byte[], ByteArrayInputAdapter, byte[], ByteArrayOutputAdapter> createByteArrayClient(
			SULClientConfiguration configuration) throws IOException {
	...
	public static SULClient<Boolean[], InputAdapter<Boolean[]>, Boolean[], OutputAdapter<Boolean[]>> createBooleanArrayClient(
			SULClientConfiguration configuration) throws IOException {
	...
	public static SULClient<Word<Boolean>, InputAdapter<Word<Boolean>>, Word<Boolean>, OutputAdapter<Word<Boolean>>> createBooleanWordClient(
			SULClientConfiguration configuration) throws IOException {
	...
	public O step(I input) {
		...
		try {
			...
			int code = in.read();
			if (code != 0) {
				logger.error(String.format("Response from server was not valid. Expected {0}, got {%d}", code));
