...
public class ExampleSUL {
    public static MealySimulatorSUL<Word<Boolean>, Object> createExample(){
        ...
        var machine = AutomatonBuilders.newMealy(alphabet).withInitial("s0")
                .from("s0").on(Word.fromSymbols(false, false)).withOutput(Word.fromSymbols(false,true)).to("s0")
                .from("s0").on(Word.fromSymbols(false,true)).withOutput(Word.fromSymbols(false,true)).to("s0")
                .from("s0").on(Word.fromSymbols(true, true)).withOutput(Word.fromSymbols(false, false)).to("s0")
                .from("s0").on(Word.fromSymbols(true, false)).withOutput(Word.fromSymbols(true, true)).to("s1")
                .from("s1").on(Word.fromSymbols(false,true)).withOutput(Word.fromSymbols(false,true)).to("s0")
